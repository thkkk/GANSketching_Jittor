...
def weights_init_normal(m):
    ...
    if classname.find('Conv') != -1:
        m.weight.data = init.gauss(m.weight.data.shape, "float32", 0.0, 0.02)
    elif classname.find('Linear') != -1:
        m.weight.data = init.gauss(m.weight.data.shape, "float32", 0.0, 0.02)
    elif classname.find('BatchNorm2d') != -1:
        m.weight.data = init.gauss(m.weight.data.shape, "float32", 1.0, 0.02)
...
def weights_init_xavier(m):
    classname = m.__class__.__name__
    if classname.find('Conv') != -1:
        m.weight.data = init.xavier_gauss(m.weight.data.shape, "float32", gain=0.02)
    elif classname.find('Linear') != -1:
        m.weight.data = init.xavier_gauss(m.weight.data.shape, "float32", gain=0.02)
    elif classname.find('BatchNorm2d') != -1:
        m.weight.data = init.gauss(m.weight.data.shape, "float32", 1.0, 0.02)
...
class GANLoss(nn.Module):
    ...
    def get_target_tensor(self, x, target_is_real):
        if target_is_real:
            if self.real_label_var is None or self.real_label_var.numel() != x.numel():
                self.real_label_var = jt.full_like(x, self.real_label)
                self.real_label_var.stop_grad()
            target_tensor = self.real_label_var
        else:
            if self.fake_label_var is None or self.fake_label_var.numel() != x.numel():
                self.fake_label_var = jt.full_like(x, self.fake_label)
                self.fake_label_var.stop_grad()
            target_tensor = self.fake_label_var
        return target_tensor

    def execute(self, x, target_is_real):
        return self.loss(x, self.get_target_tensor(x, target_is_real))
