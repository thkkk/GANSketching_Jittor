    ...
    def get_target_tensor(self, input, target_is_real):
        if target_is_real:
            if self.real_label_tensor is None:
                ...
                self.real_label_tensor = jt.zeros(1, dtype=self.Tensor) + self.real_label
                self.real_label_tensor.requires_grad = False
            return self.real_label_tensor.expand_as(input)
        else:
            if self.fake_label_tensor is None:
                # self.fake_label_tensor = self.Tensor(1).fill_(self.fake_label)
                self.fake_label_tensor = jt.zeros(1, dtype=self.Tensor) + self.fake_label
                self.fake_label_tensor.requires_grad = False
            return self.fake_label_tensor.expand_as(input)

    def get_zero_tensor(self, input):
        if self.zero_tensor is None:
            self.zero_tensor = jt.zeros(1, dtype=self.Tensor)
...
class RegularizeD(nn.Module):
    def execute(self, real_pred, real_img):
        ...
        grad_real = jt.grad(outputs, real_img)
...
class WeightLoss(nn.Module):
    def __init__(self, params):
        ...
        self.ref_weights = [
            jt.Var(p).stop_grad() for p in params
        ]
