...
class PixelNorm(nn.Module):
    ...
    def execute(self, input):
        return input * (1 / jt.sqrt(jt.mean(input ** 2, dim=1, keepdims=True) + 1e-8))


def make_kernel(k):
    # k could be a list or a numpy array
    k = jt.float32(k)
...
class EqualLinear(nn.Module):
    ...
    def execute(self, input):
        if self.activation:
            out = nn.matmul(input, (self.weight * self.scale).transpose())
            out = fused_leaky_relu(out, self.bias * self.lr_mul)

        else:
            out = nn.matmul(
                input, (self.weight * self.scale).transpose()
...
def myConvTranspose(x, weight, in_channels, out_channels, kernel_size, stride=1,
    ...
    # weight = init.invariant_uniform((in_channels, out_channels//groups) + kernel_size, dtype="float")
    if bias:
        fan = 1
        for i in weight.shape[1:]:
            fan *= i
        bound = 1 / math.sqrt(fan)
        bias = init.uniform([out_channels], dtype="float", low=-bound, high=bound)
    else:
        bias = None
...
class ModulatedConv2d(nn.Module):
    ...
    def execute(self, input, style):
        ...
        if self.upsample:
            ...
            out = myConvTranspose(
                input, weight, in_channel * batch, self.out_channel * batch, self.kernel_size,
                stride=2, padding=0, groups=batch
            )
...
class NoiseInjection(nn.Module):
    ...
    def execute(self, image, noise=None):
        if noise is None:
            ...
            noise = jt.empty((batch, 1, height, width)).gauss_()
...
class ToRGB(nn.Module):
    ...
    def execute(self, input, style, skip=None):
        out = self.conv(input, style)  # [50,512,3,3,] -> [50,3,3,3,]
        out = out + self.bias
...
class Generator(nn.Module):
    def __init__(
        ...
        for layer_idx in range(self.num_layers):
            ...
            setattr(self.noises, f"noise_{layer_idx}", x)  # add '_'!
    ...
    def execute(
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(
                list(self.convs.values())[::2], list(self.convs.values())[1::2],
...
class Discriminator(nn.Module):
    ...
    def execute(self, input):
        ...
        stddev: jt.Var = out.view(
            group, -1, self.stddev_feat, channel // self.stddev_feat, height, width
        )

        # calculate unbiased std at dim 0
        mean_stddev = stddev.mean(0, keepdims=True)
        stddev = (stddev - mean_stddev) ** 2
        n_1_ = stddev.shape[0]
        stddev = jt.sqrt(stddev.sum(0) / n_1_ + 1e-8)

        stddev = jt.squeeze(stddev.mean((2, 3, 4), keepdims=True), 2)
