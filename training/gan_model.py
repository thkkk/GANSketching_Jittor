...
class GANModel(nn.Module):
    ...
    def create_optimizers(self, opt):
        ...
        optimizer_G = nn.Adam(G_params, lr=G_lr, betas=(G_beta1, G_beta2))
        optimizer_D = nn.Adam(D_params, lr=D_lr, betas=(D_beta1, D_beta2))
    ...
    def save(self, iters):
        ...
        jt.save(self.netG.state_dict(), save_path + "G.jt")
        jt.save(self.netD_sketch.state_dict(), save_path + "D_sketch.jt")
        if self.opt.l_image > 0:
            jt.save(self.netD_image.state_dict(), save_path + "D_image.jt")

    def load(self, iters):
        load_path = os.path.join(self.opt.checkpoints_dir, self.opt.name, f"{iters}_net_")
        state_dict_g = jt.load(load_path + "G.jt")
        self.netG.load_state_dict(state_dict_g)

        state_dict_d_sketch = jt.load(load_path + "D_sketch.jt")
        self.netD_sketch.load_state_dict(state_dict_d_sketch)
        if self.opt.l_image > 0:
            state_dict_d_image = jt.load(load_path + "D_image.jt")
    ...
    def generate_fake(self, batch_size=None, style_mix=True, return_latents=False):
        ...
        noises = mixing_noise(batch_size, self.opt.z_dim, style_mix_prob)
...
def mixing_noise(batch, latent_dim, prob):
    ...
    if prob > 0 and random.random() < prob:
        return jt.unbind(jt.randn(2, batch, latent_dim), 0)
    else:
